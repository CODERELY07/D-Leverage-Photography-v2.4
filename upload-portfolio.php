<?php
    ...
    $imageCount = 0;
    $countRes = $db->query("SELECT COUNT(*) AS cnt FROM image");
    if ($countRes) {
        $imageCount = (int) $countRes->fetch_assoc()['cnt'];
    }
?>
            ...
            if (deleteBtn) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        deleteImage(deleteBtn, filename);
                    }
                });
            }
        });
        ...
        // Open file input on image click
            ...
            // Disabling the button must happen AFTER this tick, not directly in
            // the submit handler — a submit button disabled during its own
            // 'submit' event gets dropped from the request the browser actually
            // sends, so upload.php never sees $_POST['upload'] and silently no-ops.
